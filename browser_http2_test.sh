...
# ======================================================
# Browser HTTP/2 File Download Performance Test Script
# ======================================================
...
OUTPUT_DIR="browser_http2_data"
PROFILE_DIR="/tmp/chrome-http2"
OUTPUT_FILE="browser_http2_file_test_results.csv"
...
for FILE_SIZE in "${FILE_SIZES_MB[@]}"; do 
    ...
    for BANDWIDTH in "${BANDWIDTHS[@]}"; do 
        ...
        echo "Downloading ${FILE_NAME} via HTTP/2 at ${BANDWIDTH}"
        for ((i=1; i<=ITERATIONS; i++)); do
            echo "Iteration $i: Downloading ${FILE_NAME}"
            #Measure start time
            START_TIME=$(date +%s.%N)
            #Start browser with HTTP/2 settings
            #($BROWSER_BIN --headless --disable-gpu --disable-quic --no-sandbox --enable-logging --log-level=0 --v=1 --enable-precise-memory-info --enable-benchmarking --enable-net-benchmarking --user-data-dir=/tmp/chrome-http2_$i --ignore-certificate-errors --remote-debugging-port=9222 "https://${SERVER}:${PORT}/${FILE_NAME}" > perf_log.json 2>&1 &) & BROWSER_PID=$!
            timeout 10s wait $BROWSER_BIN --headless --disable-gpu --disable-quic --no-sandbox --enable-logging --log-level=0 --v=1 --enable-precise-memory-info --enable-benchmarking --enable-net-benchmarking --user-data-dir=/tmp/chrome-http2_$i --allow-insecure-localhost --ignore-certificate-errors "https://${SERVER}:${PORT}/${FILE_NAME}" > "${OUTPUT_DIR}/perf_${FILE_SIZE}_${BANDWIDTH}_${i}.json" 2>&1 &
            BROWSER_PID=$!
            #sleep 1
            ...
            #if [ -n "$CPU_AVG" ]; then 
                # Extract CPU usage
                #CPU_USER_AVG=$(echo "$CPU_AVG" | awk '{print $7}')
                #CPU_SYS_AVG=$(echo "$CPU_AVG" | awk '{print $8}')
            #else
                #CPU_USER_AVG=0.0
                #CPU_SYS_AVG=0.0
            #fi
            ...
            echo "${FILE_NAME},${BANDWIDTH},${i},${DURATION},${LOAD_SEC},${THROUGHPUT},${CPU_USER_AVG}" >> "$OUTPUT_FILE"
            #rm -f "$CPU_JSON_FILE" perf_log.json
            rm -rf "/tmp/chrome-http2_$i"
        done
    done
done
reset_bandwidth_limit
echo "HTTP/2 browser test completed. Results saved to $OUTPUT_FILE"
