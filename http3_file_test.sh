...
# ===================================================================
# This script tests downloading files of various sizes using HTTP/3
# ===================================================================

SERVER="192.168.3.220"
PORT="4433"
INTERFACE="eth0"
OUTPUT_DIR="raw3_data"
OUTPUT_FILE="http3_file_test_results.csv"
...
for FILE_SIZE in "${FILE_SIZES_MB[@]}"; do
    ...
    for BANDWIDTH in "${BANDWIDTHS[@]}"; do
        ...
        echo "Testing HTTP/3 for ${FILE_NAME} at bandwidth ${BANDWIDTH}"
        for ((i=1; i<=ITERATIONS; i++)); do
            echo "Iteration $i: Downloading ${FILE_NAME}"
            #Run curl and capture PID
            ($CURL_BIN -w "TOTAL_TIME:%{time_total}\n" -o /dev/null -k --http3 "https://${SERVER}:${PORT}/${FILE_NAME}") & CURL_PID=$!
            ...
            #CPU_USER_AVG=$(awk '/^[0-9]/ {sum+=$8; n++} END {if (n>0) print sum/n; else print 0}' "$PIDSTAT_FILE")
            #CPU_SYS_AVG=$(awk '/^[0-9]/ {sum+=$9; n++} END {if (n>0) print sum/n; else print 0}' "$PIDSTAT_FILE")
